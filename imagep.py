...
j=cv2.imread('NewCoinsBrightDark.jpg',0)
p=cv2.imread('NewCoinsBrightDark.jpg',-1)
...
def calc_Range_BW(array):
	...
	for a in range(0,len(array[0])-1) :
		...
		for b in range(0, len(array)) :
			big=max(big,array[b][a])
			small=min(small,array[b][a])
		rangeBW[0][a]+=big+0.9*(big-small)
		rangeBW[1][a]+=small-0.9*(big-small)
...
circles=cv2.HoughCircles(j,cv.CV_HOUGH_GRADIENT,1,12,param1=100,param2=15,minRadius=7,maxRadius=12)
...
number=0
radius=0
for i in circles[0, smile emoticon :
